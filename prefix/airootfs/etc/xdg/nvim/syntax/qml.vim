...
syn keyword qmlModuleIdentifier  QtQuick
syn keyword qmlModuleIdentifier  QtGraphicalEffects
...
syn keyword qmlReserved          abstract boolean byte char class debugger enum export extends final float goto implements import as interface long native package pragma private protected public short static super synchronized throws transient volatile
...
if version >= 508 || !exists("did_qml_syn_inits")
    ...
    "     command -nargs=+ hi link hi link <args>
    " else
    "     command -nargs=+ hi link hi def link <args>
    " endif
    hi link qmlComment Comment
    hi link qmlLineComment Comment
    hi link qmlCommentTodo Todo
    hi link qmlSpecial Special
    hi link qmlStringS String
    hi link qmlStringD String
    hi link qmlStringT String
    hi link qmlCharacter Character
    hi link qmlModuleIdentifier Function
    hi link qmlNumber Number
    hi link qmlConditional Conditional
    hi link qmlRepeat Repeat
    hi link qmlBranch Conditional
    hi link qmlOperator Operator
    hi link qmlJsType Type
    hi link qmlType Type
    hi link qmlObjectLiteralType Type
    hi link qmlStatement Statement
    hi link qmlFunction Function
    hi link qmlArrowFunction Function
    hi link qmlBraces Function
    hi link qmlError Error
    hi link qmlNull Special
    hi link qmlBoolean Boolean
    hi link qmlRegexpString String
    hi link qmlIdentifier Identifier
    hi link qmlLabel Label
    hi link qmlException Exception
    hi link qmlMessage Keyword
    hi link qmlGlobal Keyword
    hi link qmlReserved Include
    hi link qmlDebug Debug
    hi link qmlConstant Label
    hi link qmlBindingProperty Label
    hi link qmlDeclaration Identifier
endif
